...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isValidName(cleanName)) { setError('Name must be at least 2 characters.'); return; }
    if (!isValidEmail(cleanEmail)) { setError('Enter a valid email'); return; }
    if (!isValidPhone(cleanPhone)) { setError('Enter valid phone number (e.g.: 01XXXXXXXXX)'); return; }
      ...
      <div className="min-h-screen flex items-center justify-center bg-secondary/30 px-4">
        <div className="w-full max-w-sm animate-fade-in">
          <div className="rounded-xl border bg-card p-6 shadow-sm text-center">
            ...
            <h2 className="font-display text-xl font-bold mb-2">Verify Email</h2>
