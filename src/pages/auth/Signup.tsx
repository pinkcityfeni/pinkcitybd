...
export default function Signup() {
  ...
  const handleGoogleSignup = async () => {
    ...
    if (result.error) {
      setError('Google সাইনআপ ব্যর্থ হয়েছে');
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isValidName(cleanName)) { setError('নাম কমপক্ষে ২ অক্ষর হতে হবে'); return; }
    if (!isValidEmail(cleanEmail)) { setError('সঠিক ইমেইল দিন'); return; }
    if (!isValidPhone(cleanPhone)) { setError('সঠিক ফোন নম্বর দিন (যেমন: 01XXXXXXXXX)'); return; }
      ...
      <div className="min-h-screen flex items-center justify-center bg-secondary/30 px-4">
        <div className="w-full max-w-sm animate-fade-in">
          <div className="rounded-xl border bg-card p-6 shadow-sm text-center">
            ...
            <h2 className="font-display text-xl font-bold mb-2">ইমেইল যাচাই করুন</h2>
    ...
    <div className="min-h-screen flex items-center justify-center bg-secondary/30 px-4">
      <div className="w-full max-w-sm animate-fade-in">
        ...
        <div className="rounded-xl border bg-card p-6 shadow-sm">
          ...
          <Button variant="outline" className="w-full rounded-lg h-10 mb-4 gap-2" onClick={handleGoogleSignup}>
            ...
            <div>
              ...
              {password.length > 0 && (
                <div className="flex items-center gap-1 mt-1">
                  {passwordCheck.valid ? <CheckCircle2 className="h-3 w-3 text-green-500" /> : <XCircle className="h-3 w-3 text-destructive" />}
