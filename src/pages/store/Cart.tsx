...
export default function Cart() {
  ...
  if (cart.length === 0) return (
    <div className="container mx-auto px-4 py-20 text-center animate-fade-in">
      ...
      <h2 className="font-display text-xl font-bold mb-2">কার্ট খালি</h2>
      <p className="text-sm text-muted-foreground mb-6">সুন্দর পণ্য যোগ করুন</p>
      <Button asChild className="rounded-full px-6"><Link to="/shop">শপিং শুরু করুন</Link></Button>
    </div>
  );

  return (
    <div className="container mx-auto px-4 py-6 animate-fade-in">
      <h1 className="font-display text-xl font-bold mb-4">কার্ট <span className="text-muted-foreground font-sans text-xs font-normal">({itemCount}টি পণ্য)</span></h1>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
              ...
              <div key={item.product.id} className="rounded-2xl border bg-card p-3 space-y-2">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">৳{item.product.price.toFixed(0)} প্রতিটি</p>
                    {maxStock < 10 && <p className="text-[10px] text-destructive">{maxStock}টি বাকি</p>}
                  </div>
                ...
                <div className="flex items-center justify-between pl-[60px]">
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      className="h-7 w-7 rounded-full border flex items-center justify-center hover:bg-muted transition-colors"
                      onClick={() => {
                        if (item.quantity >= maxStock) { toast.error('সর্বোচ্চ স্টক'); return; }
                        updateCartQty(item.product.id, item.quantity + 1);
                      }}
                    >
        ...
        <div className="rounded-2xl border bg-card p-4 h-fit space-y-3">
          <h3 className="font-display font-bold text-sm">অর্ডার সারসংক্ষেপ</h3>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-muted-foreground">সাবটোটাল</span><span className="font-medium">৳{total.toFixed(0)}</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">ডেলিভারি চার্জ</span><span className="text-xs text-muted-foreground">চেকআউটে দেখুন</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">পয়েন্ট</span><span className="text-primary font-medium">+{points} pts</span></div>
          </div>
          <div className="border-t pt-3 flex justify-between font-bold">
            <span className="font-display">সাবটোটাল</span>
          ...
          <Button asChild className="w-full rounded-full shadow-lg shadow-primary/20" size="lg">
            <Link to="/checkout">চেকআউট <ArrowRight className="ml-2 h-4 w-4" /></Link>
          </Button>
          <Button asChild variant="ghost" className="w-full rounded-full" size="sm">
            <Link to="/shop">শপিং চালিয়ে যান</Link>
          </Button>
        </div>
      </div>
    </div>
