      ...
      <div className="grid lg:grid-cols-3 gap-4">
            ...
            return (
              <div key={item.product.id} className="rounded-xl border bg-card p-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">Tk {item.product.price.toFixed(0)}</p>
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <p className="font-bold text-sm" style={{ fontFamily: 'DM Sans, sans-serif' }}>Tk {(item.product.price * item.quantity).toFixed(0)}</p>
                </div>
              </div>
            );
          })}
        ...
        <div className="rounded-xl border bg-card p-4 h-fit space-y-3">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-muted-foreground">{t('cart.subtotal')}</span><span className="font-medium">Tk {total.toFixed(0)}</span></div>
          ...
          <div className="border-t pt-3 flex justify-between font-bold text-base"><span>{t('checkout.total')}</span><span style={{ fontFamily: 'DM Sans, sans-serif' }}>Tk {total.toFixed(0)}</span></div>
