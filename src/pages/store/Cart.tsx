...
export default function Cart() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 animate-fade-in">
      <h1 className="font-display text-2xl font-bold mb-6">Shopping Cart <span className="text-muted-foreground font-sans text-sm font-normal">({itemCount} items)</span></h1>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-2">
          {cart.map(item => {
            ...
            return (
              <div key={item.product.id} className="rounded-2xl border bg-card p-4 flex items-center gap-4 hover:shadow-sm transition-shadow">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {maxStock < 10 && <p className="text-[10px] text-destructive mt-0.5">{maxStock} left</p>}
                </div>
                ...
                <p className="font-display font-bold text-sm w-16 text-right">${(item.product.price * item.quantity).toFixed(2)}</p>
                <button onClick={() => removeFromCart(item.product.id)} className="p-1.5 rounded-full text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors">
        ...
        <div className="rounded-2xl border bg-card p-5 h-fit space-y-4">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between"><span className="text-muted-foreground">Shipping</span><span>{total > 50 ? <span className="text-success">Free</span> : '$5.00'}</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">Points earned</span><span className="text-primary">+{points} pts</span></div>
          </div>
          <div className="border-t pt-3 flex justify-between font-bold">
            <span className="font-display">Total</span>
            <span className="font-display text-primary text-lg">${(total + shipping).toFixed(2)}</span>
          </div>
